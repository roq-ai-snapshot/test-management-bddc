...
import { TestCaseListPage as ShowCaseList } from 'pages/test-cases';
...
function DashboardPage() {
  ...
  const testCaseStat = useSWR<PaginatedInterface<TestCaseInterface>>(
    () => `/test-cases`,
    () => getTestCases({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Test Cases',
      result: testCaseStat?.data?.totalCount,
